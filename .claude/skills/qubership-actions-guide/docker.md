...
| Do you have a Docker config file? If yes — what is its path?                                | If yes — read it. If no — ask if they want one, and confirm `.qubership/docker.cfg` as the default path (see *Config file* below). The filename is arbitrary; the path is passed via `file-path` input to `docker-config-resolver`. |
