...
def replace_tag_regexp(image_str, tag_re):
    ...
    if tag_re.startswith("#"):
        try:
            ...
            tags = subprocess.run(
                f"skopeo list-tags docker://{image_str} | jq -r '.Tags[]'",
                shell=True,
                text=True,
                check=True,
                capture_output=True,
...
def set_image_versions(config_file, tag, chart_version, method):
    ...
    for chart in data["charts"]:
        ...
        for image in chart["image"]:
            ...
            if method == "parse":
                image_ver = replace_env_variables(
                    image.split(":")[1].replace("${tag}", tag).replace("${release}", tag),
                )
                image_ver = replace_tag_regexp(search_str, image_ver)
            print(f"{values_file}: Updating {search_str} version to {image_ver}")
            os.system(
                f"sed -i 's|{search_str}:[a-zA-Z0-9._-]*|{replace_str}:{image_ver}|' {values_file}",
            )
            # Check if image key exists in values.yaml
            replacements = subprocess.run(
                f"grep -o '{replace_str}' {values_file} | wc -l",
                shell=True,
                check=True,
                capture_output=True,
                text=True,
...
def main():
    ...
    parser.add_argument(
        ...
        required=False,
        choices=["replace", "parse"],
        default="parse",
        help="Method to update image versions.",
    )
    args = parser.parse_args()

    set_image_versions(args.config_file, args.tag, args.chart_version, args.version_replace_method)
