...
- Selects the APM CLI version from `apm.lock.yaml` unless `apm-version` explicitly overrides it
- Runs `apm update --yes --target <target>` non-interactively via [microsoft/apm-action](https://github.com/microsoft/APM), using the resolved APM CLI version
...
| `apm-version` | Optional APM CLI version override | No | `""` |
...
2. Resolves the APM CLI version from `apm-version`, then `apm.lock.yaml`'s `apm_version`.
  When neither provides a version, uses the `microsoft/apm-action` default. An existing
  lockfile without `apm_version` fails before the action modifies files.
3. Sets up APM via `microsoft/apm-action` (setup-only mode).
4. Reads `apm.yml`, migrates legacy `target:` to `targets:`, and resolves the target to use.
...
5. Optionally prints runner diagnostics, `apm targets`, and dry-run update output when `debug: true`.
6. Runs `apm update --yes --target <target>` using the resolved target.
7. If `dry-run: true`, skips pull request creation after printing the update result.
8. Otherwise creates or updates a PR on branch `chore/update-apm-packages` (base: `inputs.branch`).
...
9. If `auto-merge: true` and a pull request was created (skipped in `dry-run` mode or when no
...
10. Logs the PR URL to the job summary, or reports "no changes" if nothing was updated.
...
- `apm-version` overrides the CLI version recorded in `apm.lock.yaml`. When the input is empty,
  the action uses `apm.lock.yaml`'s `apm_version`; if no lockfile exists, it uses the
  `microsoft/apm-action` default. A lockfile without `apm_version` fails before files change.
